...
df['total'] = df['process_1'] + df['process_2'] + df['process_3']
...
if st.button("Run Prediction"):
    ...
    # TOTAL DULU
    total = model.predict(input_data)[0]

    # BARU HITUNG P3
    predicted_p3 = total - p1 - p2
    ...
    if bottleneck == "Order → Warehouse":
        st.write("Improve coordination and speed up item retrieval from warehouse systems.")

    elif bottleneck == "Warehouse → Packing":
        st.write("Optimize warehouse flow and improve material handling efficiency.")

    else:
        st.write("Increase manpower or improve packing efficiency to reduce processing time.")